...
export function RelatedCiders({ ciders, currentCiderId }: RelatedCidersProps) {
  // Filter out current cider and get 4 random ciders
  ...
  return (
    <div>
      <h2 className="text-3xl font-oswald text-brand-navy uppercase mb-8 text-center">You Might Also Like...</h2>
      <div className="container mx-auto px-4 mb-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {relatedCiders.map((cider) => (
            <Link key={cider.id} href={`/ciders/${cider.slug}`} className="block text-center">
